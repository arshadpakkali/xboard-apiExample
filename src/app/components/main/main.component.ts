...
export class MainComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.http
      .get(`${environment.url}/api/product`)
  ...
  VerifyPayment(params) {
    this.paymentservice.verifyOrder(params).subscribe((params) => {
      this.fetchUser();
      console.log('Success');
    });
  }
  ...
  fetchUser() {
    ...
    this.http
      .get(`${environment.url}/api/user/get-user-by-token`, {})
      .subscribe((data: any) => {
        this.user = data;
        ...
        this.user.products.forEach((product) => {
          ...
          if (
            Math.floor(
              (new Date(product.expiry).valueOf() - new Date().valueOf()) /
                (1000 * 60 * 60 * 24)
            ) > 0
          ) {
            this.ngZone.run(() => {
  ...
  clickMe(product) {
    ...
    this.paymentservice.getOrderId(query).subscribe((response) => {
      this.payWithRazor(product, response);
    });
  ...
  logout() {
    this.http
      .post(`${environment.url}/api/user/logout`, {})
      .subscribe((res) => {
        console.log('fyfyy');
        console.log(res[0]);
