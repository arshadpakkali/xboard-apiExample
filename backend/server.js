...
mongoose.connect('mongodb://0.0.0.0:27017/users', {useNewUrlParser: true,useUnifiedTopology: true}).then((data)=>{
...
app.get('/api/resetpassword',async (req ,res, next)=>{
  try{
    if(!req.query.token){
      ...
      let verification = await jwt.verify(req.query.token, 'arstrdjjthdtdhgdtychgkhkdty');
      if(!verification) {
